...
export default function WeekOneAssessmentForm({ onSubmit, onNext, onBack, course, activityData, isCompleted }) {
	...
	useEffect(() => {
		...
		if (storedData) {
			...
			if (parsedData && parsedData.formattedData && parsedData.formattedData.week === 1) {
	...
	const questionsArrayRed = [
		...
		{
			title: 'When faced with a challenging task, what is your initial response?',
			questionList: [
		...
		{
			title: 'When planning activities with friends, what is your typical approach?',
			questionList: [
	...
	const questionsArrayBlue = [
		{
			title: 'When working on a group project, how do you typically contribute?',
			questionList: [
		...
		{
			title: 'How do you approach situations where someone is feeling upset or emotional?',
			questionList: [
	...
	const questionsArrayGreen = [
		...
		{
			title: 'In a situation where you need to make a decision, what is your priority?',
			questionList: [
		...
		try {
			...
			const formattedData = {
				...
				assessments: questionsArray.map((_, index) => ({
					answer: questionChecked[index] !== undefined ? questionChecked[index] : null, // Save the selected answer as an index or null if not selected
				})),
			};
			...
			const correctCount = formattedData.assessments.reduce((count, current, index) => {
			...
			return (
				<div style={{ justifyContent: 'start' }} className="assessment question-box py-4">
					<div className="">
						...
						<h2 style={{ color: '#5B616A' }} className="my-5 text-center mx-auto w-75">
							Before we proceed, please select the result of your personality test.
							What is your personality colour?
						</h2>
						<div className="p-3 pe-5 text-area-box two">
							<select
								style={{ padding: '16px' }}
		...
		if (questionIndex >= 0 && questionIndex < questionsArray.length) {
			return (
				<div className="assessment question-box py-4">
					...
					<div className="checkbox-questions" style={{ marginLeft: '2rem' }}>
						<ul className="p-0">
		...
		<div>
			...
			<div className="slider-indicator">
				<ul className="p-0 mt-3 mb-3">
					{Array.from({ length: getQuestionsArray().length + 1 }, (_, index) => (
						<li
							key={index}
