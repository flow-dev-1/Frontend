...
function MuitiFlipCheckBoxesFrame({ data, answers, setAnswers, setErrorMessage }) {
  ...
  const colorToBackgroundMap = {
    'maroon': { checkBg: maroonCheckBg, modalBg: maroonModalBg, colorCode: '#F46851' },
    'orange': { checkBg: orangeCheckBg, modalBg: orangeModalBg, colorCode: '#CB962C' },
    'purple': { checkBg: purpleCheckBg, modalBg: purpleModalBg, colorCode: '#8F44B5' },
    'green': { checkBg: greenCheckBg, modalBg: greenModalBg, colorCode: '#89BB3E' }
  };
    ...
    <QuestionBox>
      ...
      {showModal && selectedOption && (
        ...
        >
          ...
          >
            <div style={{ position: 'absolute', top: '10px', right: '20px', cursor: 'pointer' }} className="mr-5" onClick={closeModal}>
              ✕
            </div>
            <h2 className="mb-1 mt-1 w-100" >{selectedOption.value}</h2>
            <hr style={{ border: `0.01em solid ${colorToBackgroundMap[selectedOption.color].colorCode}` }} />
            <p style={{ color: colorToBackgroundMap[selectedOption.color].colorCode }}>{selectedOption.description}</p>
