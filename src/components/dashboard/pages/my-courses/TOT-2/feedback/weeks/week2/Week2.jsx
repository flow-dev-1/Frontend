...
function Week2({ enrollmentId, setWeekOneData }) {
  ...
  function drag1(type) {
    ...
    const indices = type === "yes" ? values.green : values.red;

    if (!Array.isArray(indices) || !activity2?.steps?.[1]?.images) return [];

    return indices
      .map((index) => activity2.steps[1].images[index])
      .filter(Boolean);
  }
  ...
  return (
    <>
      ...
      <p
        ...
        style={{ fontSize: 1 + "em" }}
      >
        {[activity2.steps[0].challenge].map((text) => (
          <>
